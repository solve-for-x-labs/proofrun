...
async function normalize(input, index, outDir) {
  ...
  const steps = (manifest.steps ?? manifest.journey?.steps ?? []).map((step, i) => ({
    ...
    // Capture provenance: lets the admin judge each frame instead of trusting it blindly.
    capture: typeof step.capture === "string" ? step.capture : null,
    captureNote: typeof step.captureNote === "string" ? step.captureNote : null
  }));
  const mediaIssues = [];
...
:root{color-scheme:dark;font:16px/1.5 system-ui;background:#07111f;color:#f4f8ff}*{box-sizing:border-box}body{margin:0}main{max-width:1800px;margin:auto;padding:24px}h1{font-size:clamp(1.5rem,3vw,2.5rem)}h2{font-size:1.2rem}button,select,a{font:inherit}button,select,.original{min-height:44px;padding:8px 12px;border:1px solid #7797b8;border-radius:8px;background:#142c48;color:inherit}button{cursor:pointer}button:disabled{opacity:.55;cursor:default}:focus-visible{outline:3px solid #ffd078;outline-offset:3px}a{color:#9edaff}.skip{position:absolute;top:-100px}.skip:focus{top:8px;background:#07111f;padding:12px;z-index:2}.muted{color:#b5c7dc}.tabs,.toolbar{display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin:16px 0}[aria-pressed=true],[aria-current=step]{background:#23648f;border-color:#b3e2ff}.layout{display:grid;grid-template-columns:240px minmax(0,1fr);gap:20px}.panel{min-width:0;background:#10243d;border:1px solid #42617e;border-radius:12px;padding:16px}ol{padding-left:24px}.step{width:100%;text-align:left;margin:4px 0;overflow-wrap:anywhere}.step small{display:block}.stage{display:flex;justify-content:center;align-items:center;min-height:55vh;background:#03070c;border-radius:8px}.stage img,.stage video{display:block;width:100%;height:auto;max-height:78vh;object-fit:contain}.notice{padding:12px;border-left:4px solid #ffc66e;background:#2a2113;color:#ffe4b6;overflow-wrap:anywhere}.capbadge{display:inline-block;margin-left:8px;padding:2px 8px;border-radius:999px;font-size:11px;font-weight:800;letter-spacing:.03em;vertical-align:middle}.cap-verified{background:#123c30;border:1px solid #4d9c7b;color:#a3f2c9}.cap-blank{background:#2a2113;border:1px solid #8a6d2f;color:#ffd27f}.cap-none{background:#33201f;border:1px solid #7a4a44;color:#ffb0a6}.capnote{margin:10px 0 0;padding:10px 12px;border-radius:8px;background:#0c1a2b;border:1px solid #2f4d6b;font-size:14px}.capnote b{color:#bfe0ff}#details{margin-top:20px;overflow-wrap:anywhere}#title,#meta{overflow-wrap:anywhere}[hidden]{display:none!important}@media(max-width:800px){main{padding:12px}.layout{grid-template-columns:1fr}.stage{min-height:30vh}#steps ol{max-height:220px;overflow:auto}.panel{padding:12px}}
</style></head><body><a class="skip" href="#review">미디어로 건너뛰기</a><main><header><p class="muted">ProofRun · 관리자 미디어 리뷰</p><h1>실제 화면과 실행 증거</h1><p>첨부된 화면·영상을 우선 검토하세요. 실행 판정은 미디어 존재나 진위를 보증하지 않습니다.</p><p class="muted" id="trust"></p></header><nav id="surfaces" class="tabs" aria-label="실행 surface"></nav><div class="layout"><nav id="steps" class="panel" aria-label="실행 단계"></nav><section id="review" class="panel" tabindex="-1" aria-labelledby="title"><h2 id="title"></h2><p id="meta" class="muted"></p><div class="toolbar"><button id="imageMode">단계 화면</button><button id="videoMode">영상</button><a id="original" class="original" target="_blank" rel="noopener">원본 열기</a></div><div id="capnote" class="capnote" hidden></div><div class="stage" id="stage"></div><div class="toolbar" id="videoTools" hidden><label for="rate">재생 속도</label><select id="rate"><option value="0.5">0.5×</option><option value="1" selected>1×</option><option value="1.5">1.5×</option><option value="2">2×</option></select><button id="seek" hidden>단계 시점으로 이동</button></div><p id="mediaNote" class="muted"></p><p id="announcement" role="status"></p><aside id="details"></aside></section></div></main><script>
...
function navigation(){
 ...
 const capBadge=p=>{const c=p.capture;if(!c)return'';if(c==='verified-real')return'<span class="capbadge cap-verified">실제화면 검증</span>';if(c==='blank-capture')return'<span class="capbadge cap-blank">렌더 전 흰 화면</span>';if(c==='none')return'<span class="capbadge cap-none">미디어 없음</span>';return''};
 $('#steps').innerHTML='<h2>실행 단계</h2><p class="muted">방향키 · Home/End로 이동, Enter로 선택</p><ol>'+(s?.steps||[]).map((p,i)=>'<li><button class="step" data-index="'+i+'" '+(i===pi?'aria-current="step"':'')+'>'+esc(p.title)+capBadge(p)+'<small>'+esc(p.status)+' · '+esc(p.action)+'</small></button></li>').join('')+'</ol>';
}
...
function show(){
 ...
 const capEl=$('#capnote');
 if(p.captureNote){capEl.hidden=false;capEl.innerHTML='<b>증거 상태 · '+esc(p.capture==='verified-real'?'실제 화면(검증됨)':p.capture==='blank-capture'?'렌더 전 흰 화면':'미디어 없음')+'</b> — '+esc(p.captureNote);}
...
navigation();show();
(function trust(){const all=B.surfaces.flatMap(s=>s.steps||[]);const c={v:0,b:0,n:0,x:0};for(const p of all){if(p.capture==='verified-real')c.v++;else if(p.capture==='blank-capture')c.b++;else if(p.capture==='none')c.n++;else c.x++;}const t=$('#trust');if(!t)return;if(c.v+c.b+c.n===0){t.textContent='단계 '+all.length+'개 · 캡처 상태 미기록(레거시)';}else{t.innerHTML='증거 신뢰 요약 · <b style="color:#a3f2c9">실제화면 검증 '+c.v+'</b> · <b style="color:#ffd27f">렌더 전 흰 화면 '+c.b+'</b> · 미디어 없음 '+c.n+(c.x?' · 미기록 '+c.x:'')+' · 총 '+all.length+'단계';}})();
</script></body></html>`;
}
